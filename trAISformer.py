...
if __name__ == "__main__":
    ...
    ## Model
    # ===============================
    encoder_model = Encoder(cf).to(device)
    ...
    # ===============================
    # Load the best model
    print(f"Loading best models from {cf.ckpt_path}...")
    checkpoint = torch.load(cf.ckpt_path)
    ...
    with torch.no_grad():
        for it, (encoder_input_density, seqs, decoder_target_seq_padded, masks, seqlens, mmsis, time_starts) in pbar:
            ...
            for i_sample in range(cf.n_samples):
                ...
                d = utils.haversine(input_coords, pred_coords) * masks # (B, L)
