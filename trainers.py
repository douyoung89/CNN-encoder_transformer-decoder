...
@torch.no_grad()
def sample(model, # Now explicitly passing the decoder model
           encoder_output, # Now explicitly passing the encoder's output,
           seqs,
           steps,
           temperature=1.0,
           sample=False,
...
class Trainer:
    ...
    def __init__(self, encoder_model, model, train_dataset, test_dataset, config, savedir=None, device=torch.device("cpu"), aisdls={},
                 INIT_SEQLEN=0):
    ...
    def save_checkpoint(self, best_epoch):
        ...
        checkpoint = {
            ...
            'decoder_state_dict': raw_model.state_dict(),
    ...
    def train(self):
        ...
        all_parameters = list(self.encoder_model.parameters()) + list(self.model.parameters())
        optimizer = torch.optim.AdamW(all_parameters, lr=config.learning_rate, betas=config.betas, weight_decay=config.weight_decay)
        ...
        def run_epoch(split, epoch=0):
            ...
            self.encoder_model.train(is_train) # Set encoder train/eval mode
            ...
            for it, (encoder_input_density, decoder_input_seq_padded, decoder_target_seq_padded, mask, seqlens, mmsis, time_starts) in pbar:
                ...
                encoder_output = self.encoder_model(encoder_input_density) # (B, T, n_embd)
                # forward the model
                with torch.set_grad_enabled(is_train):
                    if return_loss_tuple:
                        ...
                        logits, loss = model(seqs, masks=masks, with_targets=True, encoder_output=encoder_output)
                ...
                if is_train:
                    ...
                    model.zero_grad()
                    loss.backward()
                    torch.nn.utils.clip_grad_norm_(all_parameters, config.grad_norm_clip)
                    optimizer.step()
        ...
        for epoch in range(config.max_epochs):
            ...
            encoder_input_density_batch, seqs, decoder_target_seq_padded_batch, masks, seqlens, mmsis, time_starts = next(iter(aisdls["test"]))
