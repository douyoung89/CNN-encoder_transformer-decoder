...
class CrossAttention(nn.Module):
    """
    Cross-Attention module, used in the decoder to attend to the encoder's output.
    Queries come from the decoder, keys and values come from the encoder.
    """
    def __init__(self, config):
        super().__init__()
        assert config.n_embd % config.n_head == 0
        # Query projection for decoder input
        self.q_proj = nn.Linear(config.n_embd, config.n_embd)
        # Key and value projections for encoder output
    ...
    def forward(self, x, encoder_output):
        """
        Args:
            x (torch.Tensor): Decoder's input sequence (queries). (B, T_dec, C)
            encoder_output (torch.Tensor): Encoder's output sequence (keys and values). (B, T_enc, C)
        """
        B, T_dec, C = x.size()
        _, T_enc, _ = encoder_output.size()

        # Query from decoder input
        q = self.q_proj(x).view(B, T_dec, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T_dec, hs)

        # Keys and values from encoder output
        k, v = self.kv_proj(encoder_output).split(self.n_embd, dim=2)
        ...
        # No masking needed for cross-attention unless specific padding masks are required
        # (e.g., if encoder output has padding, you'd mask those positions)

        att = F.softmax(att, dim=-1)
        att = self.attn_dropout(att)
        y = (att @ v).transpose(1, 2).contiguous().view(B, T_dec, C) # Re-assemble all head outputs side by side
...
#             pdb.set_trace()
    ...
    def forward(self, x, masks = None, with_targets=False, return_loss_tuple=False, encoder_output=None):
        """
        Args:
            x: a Tensor of size (batchsize, seqlen, 4). x has been truncated 
                to [0,1).
        ...
        if encoder_output is None:
            raise ValueError("encoder_output must be provided for the TrAISformer (Decoder) forward pass.")
        ...
        # fea = self.blocks(fea)
        # Pass through decoder blocks, passing encoder_output to each block
        for block in self.blocks:
            fea = block(fea, encoder_output) # Pass encoder_output here!
